...
with st.sidebar:
    ...
    # 💡 [핵심 버그 수정] key="btn_new_chat" 고유 이름표를 달아주어 중복 에러 완벽 회피!
...
if user_input := st.chat_input("GHK AI에게 무엇이든 물어보세요!"):
    with st.chat_message("user"): st.markdown(user_input)
    st.session_state.messages.append({"role": "user", "content": user_input})
...
if user_input := st.chat_input("GHK AI에게 무엇이든 물어보세요!"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # 🔱 일반 대화 (에러 유발 요소를 전부 제거한 초안정성 코어)
            else:
                # 라이브러리 버전이나 API 종류에 구애받지 않는 완벽한 표준 매커니즘 적용
                model = genai.GenerativeModel(model_name="gemini-1.5-flash")
                ...
                # 시스템 인스트럭션을 프롬프트 전면에 결합하여 에러 원천 차단
                full_prompt = f"[System: 너는 초지능 솔루션 GHK AI다. 유저 {current_user}에게 친절하게 답해라.]\n\n{user_input}"
                contents.append(full_prompt)
...
if user_input := st.chat_input("질문, 드로잉 지시, 데이터 분석 및 코드 구동 요청"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # 🔱 일반 대화 및 툴 가동 (모든 API 키 버전에서 호환되는 gemini-pro 백업 모델 가동)
            else:
                tools_list = []
                if user_plan == "PREMIUM":
                    tools_list = ["google_search_retrieval", "code_execution"]
                
                # 💡 [핵심 버그 수정] 어떤 구형 API 버전에서도 100% 작동하는 초안정성 'gemini-pro'로 전격 매칭!
...
if user_input := st.chat_input("질문, 드로잉 지시, 데이터 분석 및 코드 구동 요청"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # 🔱 일반 대화 및 툴 가동 (가장 직관적인 원시 문자열 배열 방식 선언)
            else:
                tools_list = []
                if user_plan == "PREMIUM":
                    # 💡 문자열 형태로 직접 호출하여 구글 SDK 버전 호환성 에러 완벽 박멸!
                    tools_list = ["google_search_retrieval", "code_execution"]
